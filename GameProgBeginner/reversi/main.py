    ...
    def proc_com_turn(self):
        while True:
            if self.is_com_turn():
                ...
                if self.game_mode==2:   #対局終了していたらやめる
                    break
            else:
                break
...
class AI:
    ...
    def select_move(self, board):
        ...
        ai_level = AI_LEVEL #AIの読みの深さ
        if board.turn == BLACK: #黒の手番：すごく低い評価値からスタート
            best_eval = -10000
        else:
            best_eval = 10000
        ...
        for position in move_list:
            ...
            if board.turn == BLACK:
                if eval > best_eval:
                    #黒の手番
                    #評価値がより高い手の場合は最善手を入れ替える
                    best_eval = eval
                    best_position = position
            else:
                if eval < best_eval:
                    #白の手番
                    #評価値がより低い手の場合は最善手を入れ替える
                    best_eval = eval
                    best_position = position
...
def draw_board():
    ...
    #枠を描画
    for x in range(8):
        canvas_board.create_line(
            x*CELL_PX_SIZE, 0, x*CELL_PX_SIZE, 
            BOARD_PX_SIZE, fill="black", width=1)
    for y in range(8):
        canvas_board.create_line(
            0, y*CELL_PX_SIZE, BOARD_PX_SIZE,
            y*CELL_PX_SIZE, fill="black", width=1)
...
black_rdo0 = tkinter.Radiobutton(root, value=1, variable=black_var, text=u"コンピュータ")
black_rdo0.place(x=160, y=4)
...
white_rdo0 = tkinter.Radiobutton(root, value=1, variable=white_var, text=u"コンピュータ")
white_rdo0.place(x=160, y=24)
